package com.android.acadgild.employeedetails113.database;
...
public class DBHelper {
    ...
    //to get records of all Employees
    public EmployeeData getAllEmployee() {
        List<EmployeeData> empData = new LinkedList<EmployeeData>();

        // select Employee query
        String query = "SELECT  * FROM " + Constants.EMPLOYEE_RECORD;
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                emp.setId((cursor.getString(0)).toString());
                emp.setEmpId(cursor.getString(1).toString());
                emp.setEmpName(cursor.getString(2).toString());
                emp.setEmpAge(cursor.getString(3).toString());
                emp.setEmpPhotoImage(cursor.getBlob(4));
